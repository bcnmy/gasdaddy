...
import { EntryPoint, EntryPointSimulations__factory, IEntryPointSimulations } from "../../../typechain-types";
import { PackedUserOperation, UserOperation } from "./types";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import { TransactionRequest } from '@ethersproject/abstract-provider'
import { AbiCoder, BigNumberish, BytesLike, Contract, Signer, dataSlice, keccak256, toBeHex } from "ethers";
...
export function packAccountGasLimits (verificationGasLimit: BigNumberish, callGasLimit: BigNumberish): string {
    return ethers.concat([
        ethers.zeroPadValue(toBeHex(Number(verificationGasLimit)), 16), ethers.zeroPadValue(toBeHex(Number(callGasLimit)), 16)
    ])
}
...
export async function signUserOp (op: UserOperation, signer: Signer, entryPoint: string, chainId: number): Promise<UserOperation> {
...
export async function fillUserOp (op: Partial<UserOperation>, entryPoint?: EntryPoint, getNonceFunction = 'getNonce', nonceKey = "0"): Promise<UserOperation | undefined> {
    const op1 = { ...op }
    ...
    if (op.initCode != null && op.initCode !== "0x" ) {
      ...
      if (op1.sender == null) {
        ...
        op1.sender = await entryPoint!.getSenderAddress(op1.initCode!).catch(e => e.errorArgs.sender)
      }
      if (op1.verificationGasLimit == null) {
        ...
        op1.verificationGasLimit = Number(DefaultsForUserOp.verificationGasLimit!) + Number(initEstimate)
    }
    }
    if (op1.nonce == null) {
    ...
    if (op1.maxFeePerGas == null) {
      ...
      op1.maxFeePerGas = Number(block!.baseFeePerGas!) + Number(op1.maxPriorityFeePerGas ?? DefaultsForUserOp.maxPriorityFeePerGas)
    }
...
export async function fillAndPack (op: Partial<UserOperation>, entryPoint?: EntryPoint, getNonceFunction = 'getNonce'): Promise<PackedUserOperation | undefined> {
...
export async function fillAndSign (op: Partial<UserOperation>, signer: Signer | Signer, entryPoint?: EntryPoint, getNonceFunction = 'getNonce', nonceKey = "0"): Promise<UserOperation> {
    const provider = ethers.provider
  ...
  export async function fillSignAndPack (op: Partial<UserOperation>, signer: Signer | Signer, entryPoint?: EntryPoint, getNonceFunction = 'getNonce', nonceKey = "0"): Promise<PackedUserOperation> {
    const filledAndSignedOp = await fillAndSign(op, signer, entryPoint, getNonceFunction, nonceKey)
