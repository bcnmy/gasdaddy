 ...
 * @returns A promise that resolves to the deployed MockValidator contract instance.
 */
export async function getDeployedSponsorshipPaymaster(
  owner: string,
  entryPoint: string,
  verifyingSigner: string,
  feeCollector: string,
): Promise<BiconomySponsorshipPaymaster> {
  const accounts: Signer[] = await ethers.getSigners();
  const addresses = await Promise.all(
    accounts.map((account) => account.getAddress()),
  );

  const BiconomySponsorshipPaymaster = await ethers.getContractFactory(
    "BiconomySponsorshipPaymaster",
  );
  const deterministicSponsorshipPaymaster = await deployments.deploy(
    "BiconomySponsorshipPaymaster",
    {
      from: addresses[0],
      deterministicDeployment: true,
